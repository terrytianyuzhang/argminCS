...
getMin.softmin.LOO <- function(i, r, data, lambda, sample.mean=NULL, print.weights=FALSE){
  ...
  save_weight <- list(sample_index = i,
                      dimension_of_interest = r,
                      weights = weights)
  save_weight_file <- glue::glue("{save_weight_file_prefix}_{i}_{r}.rds")
  saveRDS(save_weight, file = save_weight_file)
